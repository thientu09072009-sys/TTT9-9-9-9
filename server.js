...
app.post("/api/auth/register", async (req,res) => {
  ...
  try {
    const r = await q(`
      ...
      RETURNING id,username,ttt,plays,biggest_win,total_won
...
app.post("/api/auth/login", async (req,res) => {
  ...
  try {
    ...
    `,[sha(t),r.rows[0].id]);
    const u = r.rows[0];
    res.json({ok:true,token:t,user:{id:u.id,username:u.username,ttt:u.ttt,plays:u.plays,biggest_win:u.biggest_win,total_won:u.total_won}});
...
app.post("/api/redeem",auth,async(req,res)=>{
  ...
  try {
    ...
    await c.query(`UPDATE redeem_codes SET uses=uses+1,active=CASE WHEN max_uses>0 AND uses+1>=max_uses THEN false ELSE active END WHERE id=$1`,[item.id]);
...
app.get("/api/admin/codes",admin,async(_req,res)=>{
  try {
    const r=await q(`
      SELECT id,code,reward,max_uses,uses,active,expires_at,created_at
      FROM redeem_codes ORDER BY id DESC LIMIT 200
    `);
    res.json({ok:true,codes:r.rows});
  } catch(e) {
    console.error("admin codes",e);
    res.status(500).json({ok:false,error:"Không tải được danh sách code."});
  }
});
...
app.post("/api/admin/codes",admin,async(req,res)=>{
  ...
  try {
    ...
    `,[code,reward,maxUses,expiresAt]);
    res.json({ok:true,code:r.rows[0]});
  } catch(e) {
    if(e.code==="23505") return res.status(409).json({ok:false,error:"Code đã tồn tại."});
    console.error("admin create code",e);
    res.status(500).json({ok:false,error:"Không tạo được code."});
  }
});

app.patch("/api/admin/codes/:code",admin,async(req,res)=>{
  const code=String(req.params.code||"").trim().toUpperCase();
  try {
    const current=await q(`SELECT * FROM redeem_codes WHERE code=$1`,[code]);
    if(!current.rowCount) return res.status(404).json({ok:false,error:"Không tìm thấy code."});

    if(req.body.resetUses===true){
      await q(`UPDATE redeem_codes SET uses=0,active=TRUE WHERE code=$1`,[code]);
    } else if(typeof req.body.active === "boolean") {
      await q(`UPDATE redeem_codes SET active=$2 WHERE code=$1`,[code,req.body.active]);
    } else {
      return res.status(400).json({ok:false,error:"Không có thao tác hợp lệ."});
    }

    const r=await q(`SELECT id,code,reward,max_uses,uses,active,expires_at,created_at FROM redeem_codes WHERE code=$1`,[code]);
    res.json({ok:true,code:r.rows[0]});
  } catch(e) {
    console.error("admin patch code",e);
    res.status(500).json({ok:false,error:"Không cập nhật được code."});
  }
});

app.delete("/api/admin/codes/:code",admin,async(req,res)=>{
  const code=String(req.params.code||"").trim().toUpperCase();
  try {
    const r=await q(`DELETE FROM redeem_codes WHERE code=$1 RETURNING id,code`,[code]);
    if(!r.rowCount) return res.status(404).json({ok:false,error:"Không tìm thấy code."});
    res.json({ok:true,deleted:r.rows[0]});
  } catch(e) {
    console.error("admin delete code",e);
    res.status(500).json({ok:false,error:"Không xóa được code."});
  }
});

/* Kết quả + thống kê game cho Admin */
app.get("/api/admin/results",admin,async(req,res)=>{
  const limit=Math.min(200,Math.max(1,Math.floor(Number(req.query.limit||100))));
  const game=String(req.query.game||"all").toLowerCase();
  if(!["all","tx","xd"].includes(game)) return res.status(400).json({ok:false,error:"Game không hợp lệ."});
  ...
  try {
    ...
    if(game==="all" || game==="tx") {
      const r=await q(`
        SELECT
          ...
          r.ends_at,
          r.status,
          r.d1,r.d2,r.d3,r.total,r.side,r.result_until,
          COALESCE(b.total_bet,0) AS total_bet,
          COALESCE(b.total_payout,0) AS total_payout,
          COALESCE(b.bet_count,0) AS bet_count,
          COALESCE(b.player_count,0) AS player_count
        FROM tx_rounds r
        LEFT JOIN LATERAL (
          SELECT
            COALESCE(SUM(amount),0)::bigint AS total_bet,
            COALESCE(SUM(win_ttt),0)::bigint AS total_payout,
            COUNT(*)::int AS bet_count,
            COUNT(DISTINCT user_id)::int AS player_count
          FROM tx_bets WHERE round_id=r.id
        ) b ON TRUE
        WHERE r.status='closed'
        ORDER BY r.id DESC LIMIT $1
      `,[limit]);
      tx=r.rows.map(x=>({
        game:"tx", roundId:Number(x.round_id), startedAt:x.started_at,
        dice:[x.d1,x.d2,x.d3], total:x.total, side:x.side,
        status:x.status, resultUntil:x.result_until,
        totalBet:Number(x.total_bet), totalPayout:Number(x.total_payout),
        houseResult:Number(x.total_bet)-Number(x.total_payout),
        betCount:Number(x.bet_count), playerCount:Number(x.player_count)
      }));
    }
    ...
    if(game==="all" || game==="xd") {
      const r=await q(`
        SELECT
          ...
          r.ends_at,
          r.status,
          r.red_count,r.white_count,r.side,r.multiplier,r.result_until,
          COALESCE(b.total_bet,0) AS total_bet,
          COALESCE(b.total_payout,0) AS total_payout,
          COALESCE(b.bet_count,0) AS bet_count,
          COALESCE(b.player_count,0) AS player_count
        ...
        LEFT JOIN LATERAL (
          SELECT
            ...
            COUNT(*)::int AS bet_count,
            COUNT(DISTINCT user_id)::int AS player_count
          FROM xd_bets WHERE round_id=r.id
        ) b ON TRUE
        WHERE r.status='closed'
        ORDER BY r.id DESC LIMIT $1
      `,[limit]);
      xd=r.rows.map(x=>({
        game:"xd", roundId:Number(x.round_id), startedAt:x.started_at,
        redCount:x.red_count, whiteCount:x.white_count,
        side:x.side, multiplier:Number(x.multiplier||0),
        status:x.status, resultUntil:x.result_until,
        totalBet:Number(x.total_bet), totalPayout:Number(x.total_payout),
        houseResult:Number(x.total_bet)-Number(x.total_payout),
        betCount:Number(x.bet_count), playerCount:Number(x.player_count)
    ...
    const all=[...tx,...xd].sort((a,b)=>new Date(b.startedAt)-new Date(a.startedAt));
    const summary={
      txRounds:tx.length, xdRounds:xd.length,
      totalRounds:tx.length+xd.length,
      totalBet:all.reduce((n,x)=>n+x.totalBet,0),
      totalPayout:all.reduce((n,x)=>n+x.totalPayout,0),
      houseResult:all.reduce((n,x)=>n+x.houseResult,0),
      totalBets:all.reduce((n,x)=>n+x.betCount,0),
      totalPlayers:all.reduce((n,x)=>n+x.playerCount,0)
    };
    res.json({ok:true,summary,tx,xd});
  } catch(e) {
    console.error("admin results",e);
    res.status(500).json({ok:false,error:"Không tải được kết quả game."});
  }
});
...
app.get("/api/admin/users",admin,async(_req,res)=>{
  try {
    const r=await q(`
      SELECT id,username,ttt,plays,biggest_win,total_won,created_at
      FROM users ORDER BY total_won DESC,biggest_win DESC,id ASC LIMIT 200
    `);
    res.json({ok:true,users:r.rows});
  } catch(e) {
    console.error("admin users",e);
    res.status(500).json({ok:false,error:"Không tải được người chơi."});
  }
});

app.get("/",(_req,res)=>res.json({ok:true,service:"TTT9-9-9",status:"online"}));
