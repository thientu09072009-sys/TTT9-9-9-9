...
app.post("/api/redeem",auth,async(req,res)=>{
  ...
  try {
    ...
    const r=await c.query(`
      ...
      FOR UPDATE
    `,[code]);
    if(!r.rowCount){await c.query("ROLLBACK");return res.status(404).json({ok:false,error:"Code không tồn tại hoặc đã khóa."});}
    const item=r.rows[0];
    if(item.max_uses > 0 && item.uses>=item.max_uses){
      await c.query("ROLLBACK");
      return res.status(409).json({ok:false,error:"Code đã hết lượt sử dụng."});
    }
    await c.query(`
      UPDATE redeem_codes
      SET uses=uses+1,
          active=CASE WHEN max_uses>0 AND uses+1>=max_uses THEN false ELSE active END
      WHERE id=$1
    `,[item.id]);
...
app.get("/api/admin/codes",admin,async(_req,res)=>{
  const r=await q(`
    SELECT id,code,reward,max_uses AS "maxUses",uses AS "usedCount",
           active,expires_at AS "expiresAt",created_at
...
app.patch("/api/admin/codes/:code",admin,async(req,res)=>{
  ...
  if(req.body && Object.prototype.hasOwnProperty.call(req.body,"resetUses")) {
    await q(`UPDATE redeem_codes SET uses=0,active=TRUE WHERE code=$1`,[code]);
  } else if(req.body && Object.prototype.hasOwnProperty.call(req.body,"active")) {
...
app.delete("/api/admin/codes/:code",admin,async(req,res)=>{
  const code=String(req.params.code||"").toUpperCase();
  await q(`DELETE FROM redeem_codes WHERE code=$1`,[code]);
  res.json({ok:true});
});
app.get("/api/admin/users",admin,async(_req,res)=>{
  const r=await q(`SELECT id,username,ttt,plays,biggest_win,total_won,created_at FROM users ORDER BY total_won DESC,id ASC LIMIT 200`);
  res.json({ok:true,users:r.rows});
});

/* Old game Cascade Arena: same UI, but TTT is stored on PostgreSQL. */
app.post("/api/cascade/spin",auth,async(req,res)=>{
  const bet=Math.floor(Number(req.body.bet));
  if(!Number.isFinite(bet)||bet<1000) return res.status(400).json({ok:false,error:"Mức cược không hợp lệ."});
  const c=await pool.connect();
  try{
    await c.query("BEGIN");
    const u=await c.query(`SELECT ttt FROM users WHERE id=$1 FOR UPDATE`,[req.user.id]);
    const balance=Number(u.rows[0].ttt);
    if(balance<bet){await c.query("ROLLBACK");return res.status(400).json({ok:false,error:"Không đủ TTT."});}
    let gain=0, cascades=0;
    const symbols=["blue","fire","dark","green","gold"];
    const grid=Array.from({length:30},()=>symbols[crypto.randomInt(0,symbols.length)]);
    for(let n=0;n<11;n++){
      let win=false;
      for(let row=0;row<6;row++){
        for(let col=0;col<=2;col++){
          const a=row*5+col;
          if(grid[a]===grid[a+1]&&grid[a]===grid[a+2]) win=true;
        }
      }
      if(!win) break;
      cascades++;
      gain += bet * [1,2,4,8,16,32,64,128,256,512,1028][Math.min(cascades-1,10)];
      for(let i=0;i<30;i++) grid[i]=symbols[crypto.randomInt(0,symbols.length)];
    }
    await c.query(`UPDATE users SET ttt=ttt-$1+$2,plays=plays+1,biggest_win=GREATEST(biggest_win,$2),total_won=total_won+$2 WHERE id=$3`,
      [bet,gain,req.user.id]);
    const nr=await c.query(`SELECT id,username,ttt,plays,biggest_win,total_won FROM users WHERE id=$1`,[req.user.id]);
    await c.query("COMMIT");
    res.json({ok:true,bet,gain,cascades,grid,user:nr.rows[0]});
  }catch(e){
    try{await c.query("ROLLBACK")}catch{}
    console.error("cascade",e);
    res.status(500).json({ok:false,error:"Không xử lý được ván Cascade."});
  }finally{c.release()}
});
